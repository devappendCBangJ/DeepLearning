...
def main(args):
    ...
    dataset = OmniglotPrototype(args.root, K_s=args.num_supports, K_q=args.num_queries, training=True, transform=transform) # 모든 class 내의 이미지 -> support set, query set으로 분할
    loader = DataLoader(dataset, args.num_classes, shuffle=True, num_workers=args.num_workers, drop_last=True,  # num_classes = batch size
                        collate_fn=omniglot_prototype_collate_fn) # 출력 : 5차원 텐서 [N = B = num_classes, K, channel = 1, height = 32, width = 32]
