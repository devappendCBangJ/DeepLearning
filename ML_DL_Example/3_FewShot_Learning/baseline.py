...
def main(args):
    ...
    for alphabet, num_classes in zip(args.alphabets, args.num_characters):
        # Build dataset
        root = f'{args.root}/omniglot/meta-test/{alphabet}'
        train_data = OmniglotBaseline(root, args.num_supports, args.num_queries, training=True, transform=T.RandomCrop((32, 32), padding=4))
        train_loader = DataLoader(train_data, args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True) # 출력 : 4차원 텐서 [N = batch_size, channel = 1, height = 32, width = 32]
